...
from linesearch import WolfeLineSearch, ExactLineSearch
from quasi_newton import BFGS, DFP, BadBroydenH, GoodBroydenG, SymmetricBroyden
from optProblem import OptProblem
...
if __name__ == "__main__":
    ...
    solver_bfgs = BFGS(tol=1e-8, max_iter=200)
    x_star, f_star, info = solver_bfgs.optimize(prob, x0=x0)

    print("BFGS result:")
    print("x* =", x_star, "f* =", f_star, "status:", info["status"], "iters:", info["iters"])

    x0 = np.array([-1.2, 1.0])
    prob = OptProblem(rosen)

    solvers = [
        ("BFGS", BFGS(tol=1e-8, max_iter=500, line_search=WolfeLineSearch())),
        ("DFP", DFP(tol=1e-8, max_iter=500, line_search=WolfeLineSearch())),
        ("BadBroydenH", BadBroydenH(tol=1e-8, max_iter=500, line_search=WolfeLineSearch())),
        ("GoodBroydenG", GoodBroydenG(tol=1e-8, max_iter=500, line_search=WolfeLineSearch())),
        ("SymmetricBroyden", SymmetricBroyden(phi=0.5, tol=1e-8, max_iter=500, line_search=WolfeLineSearch())),
    ]

    for name, solver in solvers:
        x_star, f_star, info = solver.optimize(prob, x0=x0)
        print(f"{name:16s} -> status={info['status']}, iters={info['iters']}, f*={f_star:.3e}, ||g||={info['grad_norm']:.2e}")
